...
export function renderTab(tab, { horizon = 7, snapshots = [], inspiration = [] }) {
  ...
  const sectionDefs = [
    { key: "audience-tab", title: "Audience tab competitors", note: "Competitors coming from Audience-tab/reference tracking." },
    { key: "style-picks", title: "AI avatar / up-and-coming style videos", note: "Edsel sheet, AI-avatar, real-person/clip-footage, and discovery/up-and-coming inspiration picks." },
  ];
  const grouped = sectionDefs.map((def) => ({ ...def, rows: videos.filter((v) => videoSourceSection(v) === def.key) }));
