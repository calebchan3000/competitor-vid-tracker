...
function isRelevantActionable(v, inspirationSet = new Set()) {
  ...
  if (Number(v.outlier || 0) < 3) return false;
  return true;
}
...
function dismissButton(v, slug) {
  const url = videoUrl(v.videoId);
  return `<button type="button" class="dismiss-video" data-slug="${escapeHtml(slug || "")}" data-video-id="${escapeHtml(v.videoId || "")}" data-video-title="${escapeHtml(v.title || "")}" data-video-url="${escapeHtml(url || "")}" data-video-handle="${escapeHtml(v.handle || "")}" title="Cross this title off">×</button>`;
}
...
function renderEngineTable(rows, slug, inspirationSet, { id = "", section = "" } = {}) {
  return `<div class="table-scroll"><table class="vtable engine-table" ${id ? `id="${escapeHtml(id)}"` : ""} ${section ? `data-source-section="${escapeHtml(section)}"` : ""}>
    <thead><tr>
      <th class="c-use">Use</th>
      <th class="c-dismiss">X</th>
      <th></th>
...
function videoRow(v, slug, { showNiche = false, inspirationSet = new Set() } = {}) {
  ...
  return `<tr class="vrow vrow--${t.key} ${hot ? "vrow--hot" : ""}"
    ...
    <td class="c-dismiss">${dismissButton(v, slug)}</td>
    <td class="c-thumb">${thumb}</td>
    <td class="c-title">${hot ? '<span class="flame">🔥</span> ' : ""}${title}${showNiche && v.niche ? `<span class="muted niche-tag"> · ${escapeHtml(v.niche)}</span>` : ""}${labelBadges}${addedMeta(v)}</td>
...
export function renderTab(tab, { horizon = 7, snapshots = [], inspiration = [], dismissed = [] }) {
  ...
  const sectionDefs = [
    ...
    { key: "audience-tab", title: "Audience tab competitors", note: "Audience-tab/reference competitors that are also relevant outliers ≥3× and ≥10K views." },
  ];
  ...
  const engine = videos.length
    ? `<div class="relevance-note"><strong>Actionable relevance system:</strong> hidden unless ≥${OPPORTUNITY_VIEW_FLOOR.toLocaleString("en-US")} views, ≥3× baseline, not off-niche/cooking, and not crossed off. Each subsection is capped to the strongest 10.</div>
