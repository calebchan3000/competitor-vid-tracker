...
import { listSnapshots, snapshotCount } from "./snapshots.mjs";
...
function audienceSnapshotVideos(snapshots = []) {
  const seen = new Set();
  const out = [];
  for (const snap of snapshots || []) {
    for (const v of snap.audience?.videos || []) {
      const handle = v.handle || v.channel || "";
      const key = String(v.videoId || `${handle}::${v.title || ""}`).toLowerCase();
...
function videoRow(v, slug, { showNiche = false, inspirationSet = new Set() } = {}) {
  ...
  return `<tr class="vrow vrow--${t.key} ${hot ? "vrow--hot" : ""}"
    ...
    <td class="c-title">${hot ? '<span class="flame">🔥</span> ' : ""}${title}${dismissButton(v, slug)}${showNiche && v.niche ? `<span class="muted niche-tag"> · ${escapeHtml(v.niche)}</span>` : ""}${labelBadges}${addedMeta(v)}${videoCommandForm(v, slug)}</td>
...
function renderEdselResultsSummary(tabs) {
  const batches = tabs
    .flatMap((tab) => listSnapshots(tab.slug)
      .filter((snap) => String(snap.batchId || "").includes("edsel") || /edsel/i.test(snap.sourceChannelCanonical || snap.sourceChannel || ""))
      .map((snap) => ({ ...snap, tab })))
...
export function renderTab(tab, { horizon = 7, snapshots = [], inspiration = [], dismissed = [] }) {
  ...
  const studioAudienceVideos = audienceSnapshotVideos(snapshots)
    .filter((v) => v.videoId)
  ...
  const sectionDefs = [
    ...
    { key: "audience-snapshot", title: "Audience tab competitors — YouTube Studio videos", note: "Exact videos from the uploaded YouTube Studio Audience tab screenshots — shown in full before broader tracked competitors." },
