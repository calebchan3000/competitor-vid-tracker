...
export function renderTab(tab, { horizon = 7, snapshots = [], inspiration = [], dismissed = [] }) {
  ...
  // Edsel Picks is an aggregate/reference library, not a recency dashboard. Show
  // the complete list by default so Jonathan can scan every Edsel row in one tab.
  const h = isEdselPicksTab ? 0 : requestedHorizon;
  const inHorizon = (v) => {
    if (h === 0) return true;
    const age = daysAgo(v.publishDate);
  ...
  const studioAudienceVideos = audienceSnapshotVideos(snapshots, tab)
    ...
    .filter((v) => !dismissedSet.has(videoKey(v)) && !(videoUrl(v.videoId) && dismissedSet.has(videoUrl(v.videoId))));
  const hotCount = (isEdselPicksTab ? videos : actionableVideos).filter((v) => isHighlighted(v.outlier)).length;
  const filterCountVideos = isEdselPicksTab
  ...
  const engine = renderedVideos.length
    ? `<div class="relevance-note"><strong>${isEdselPicksTab ? "Full Edsel picks library" : "Actionable relevance system"}:</strong> ${isEdselPicksTab ? "Every Edsel-sheet video in this aggregate tab is shown, grouped first by source niche and then by Edsel section/style. No 7-day filter, relevance pruning, or top-10 cap is applied here." : `exact YouTube Studio Audience-tab videos are shown first when screenshots have structured data and pass the spokesperson Audience screen (${audienceSnapshotViewFloor(tab) ? `≥${formatInt(audienceSnapshotViewFloor(tab))} views for ${tab.niche}` : "no extra view floor for this niche"}). Broader rows are hidden unless ≥${OPPORTUNITY_VIEW_FLOOR.toLocaleString("en-US")} views and relevant; Edsel/style picks require ≥3× baseline; other audience/reference outliers include ≥1.5×. Fox News/Fox Business are capped.`}</div>
      <div class="channel-filter" id="channel-filter">
        ...
        ${worthChannels.length ? `<div class="filter-pills filter-pills--worth">${worthChannels.map((c) => filterButton(c, "channel-filter-pill--worth")).join("")}</div>` : `<div class="muted filter-empty">No standout channels in this horizon yet.</div>`}
        ${renderLabelFilterPills(renderedVideos)}
        <div class="filter-head filter-head--secondary"><span>Other tracked channels</span><span class="muted">multi-select</span></div>
        <div class="filter-pills">
