...
export function renderTab(tab, { horizon = 7, snapshots = [], inspiration = [], dismissed = [] }) {
  ...
  const channelCounts = [...actionableVideos.reduce((m, v) => {
    const handle = v.handle || "—";
    const rec = m.get(handle) || { handle, count: 0, hot: 0, eligibleHot: 0, maxOutlier: 0, eligibleMaxOutlier: 0, topTitle: "", eligibleTopTitle: "", opportunity: 0, eligibleOpportunity: 0, maxOpportunityOutlier: 0, eligibleMaxOpportunityOutlier: 0, topOpportunityTitle: "", eligibleTopOpportunityTitle: "" };
    rec.count += 1;
    const outlier = Number(v.outlier || 0);
  ...
  const sectionDefs = [
    ...
    { key: "audience-tab", title: "Audience tab competitors", note: "Audience-tab/reference competitors that are also relevant outliers ≥1.5× and ≥10K views." },
  ];
  const grouped = sectionDefs.map((def) => ({ ...def, rows: actionableVideos.filter((v) => videoSourceSection(v) === def.key).slice(0, 10) }));
  ...
  const engine = videos.length
    ? `<div class="relevance-note"><strong>Actionable relevance system:</strong> hidden unless ≥${OPPORTUNITY_VIEW_FLOOR.toLocaleString("en-US")} views and relevant. Edsel/style picks require ≥3× baseline; Audience tab competitors include ≥1.5× outliers. Each subsection is capped to the strongest 10.</div>
