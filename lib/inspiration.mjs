...
export function listInspiration(slug) {
  try {
    ...
    return { slug: slugify(slug), updated: parsed.updated || null, items, dismissed };
  } catch (err) {
...
export function setInspiration(slug, item, selected = true) {
  ...
  const payload = { slug: slugify(slug), updated: new Date().toISOString(), items: next, dismissed: current.dismissed || [] };
...
export function setDismissed(slug, item, dismissed = true) {
  ...
  const payload = { slug: slugify(slug), updated: new Date().toISOString(), items: nextItems, dismissed: nextDismissed };
  fs.writeFileSync(fileFor(slug), `${JSON.stringify(payload, null, 2)}\n`);
  return payload;
}
